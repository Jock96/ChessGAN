...
EVAL AlphaBeta(EVAL alpha, EVAL beta, int depth, int ply, bool isNull)
{
	...
	if (!isNull && !onPV && !inCheck && !lateEndgame && depth >= 2)
	{
		...
		if (nullScore >= beta)
		{
			RecordHash(0, beta, depth, ply, HASH_BETA);
			return beta;
		}
	}
	...
	bool isCutNode = (pEntry != NULL && pEntry->m_type == HASH_BETA);

	if (onPV && hashMove == 0 && depth > 4)
	{
		AlphaBeta(alpha, beta, depth - 2, ply, isNull);
		if (!g_pv[ply].empty())
			hashMove = g_pv[ply][0];
	}
	else if (isCutNode && hashMove == 0 && depth > 4)
	{
		AlphaBeta(alpha, beta, depth - 2, ply, isNull);
		if (!g_pv[ply].empty())
			hashMove = g_pv[ply][0];
...
Move StartSearch(const Position& pos, U8 flags)
{
	...
	Move bestMove = FirstLegalMove(s_pos);
	if (bestMove == 0)
		return 0;
